...
# number of # indicate how big the test is in your Knit
...
data%>%
  colnames()
```
# ``` specific to dplyr functions, mark where the code begin and ends so other people can rerun it
...
```{r}
subset <- data %>%
  select(-geo_accession, -characteristics_ch1, -organism_ch1)
  
```
# will remove just the column with - sign
...
We spent a lot of time in lecture explaing  what RNAseq and FPKM is lets use these data. 
Subset the dataframe to include only genes with more 200 FPKM as value and are from not cancerous tissue. Get the dimensions of this new dataframe to a new variable called size_not_cancer_abundant and view it. NOTE: not cancer is stored under *characteristics_ch1* and the value associated to not cancer is 	*tissue: normal breast tissue*
...
```{r}
data %>%
  summarize(average_exp = mean(FPKM), max_value = max(FPKM), min_value = min(FPKM), standard_deviation = sd(FPKM))
```
...
data %>%
  filter( gene == 'BRCA1'| gene == 'BRCA2') %>%
  group_by(gene, metastasis.ch1, tissue.ch1, FPKM) %>%
  summarize(avg = mean(FPKM))
```
```
...
```{r}
#mestatasis_df <- data %>%
#    rename(mestatasis = mestatasis.ch1)
