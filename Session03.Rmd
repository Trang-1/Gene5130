...
```{r}
  EnhancedVolcano(sigs.df,
    ...
    y = 'padj',
    title = 'Volcano plot',
    selectLab = c('VCAM1','DNM1','BRCA1',
                  'ALOX15B','PER1','SPARCL1','FKBP5','PHC2','ARHGEF2'),
...
```
Purpose of the code: this includes all the biological changes, for the project, you can select depending on what you want (just over expressed process)

```{R}
diff.df <- as.data.frame(sigs.df)
diff.df <- diff.df %>%
  filter(padj < 0.05, log2FoldChange >2, baseMean > 50)
