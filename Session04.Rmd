...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("C:/Users/Tranga N/Desktop/BioR")) 
```
...
```{r}
setwd("C:/Users/Trang N/Desktop/BioR")
```
...
gsG$allOK # this will tell you if all of the items are okay 
gsG$goodGenes #not all of the genes are meeting the standard
gsG$goodSamples
...
```{r}
?hclust
temp_tree= hclust(dist(t(counts)), method= 'complete')
plot(temp_tree)
```
...
sft <- pickSoftThreshold(norm_gene_exp, 
                  ...
                  verbose = 2)
```
verbose = how much of the output you want to see
want low mean = low connection -> increase powerVector
...
traits
```
y axis = height, correlation between 2 points, edges line 
```{r}
new_pheno
```

```{r}
correlation = cor(bwm$MEs, traits, use= 'p')
```


```{r}
pvalues= corRvalueStudent(correclations, 31)
```
