...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("C:/Users/Trang N/Desktop/BioR")) 
```
...
design is your experimental design = what to be compared 
```{r}
...
we want to filter where dds count across the row is >10, saved as a boolean
and saved as file called subset_dds
dim: 19083 8 
where 8 is the dimention of the matrix (columns),
19083 is how many matched the criteria 
```{r}
...
ref = 'control' set what is the control in your project
in case your control is not named 'control'
```{r}
...
#baseMean = average
#log Fold change (-) down regulated in treatment, (+) upregulated in tx
#pvalue = raw p value
#padj = new p value after correction
```{r}
...
How many genes are upregulaed??  1258
How many are downregulated?? 949
...
```
results saved as res -> get summary -> Volcano Plot shows the statistical significance of the difference relative to the magnitude of difference for every single gene in the comparison
red = stat significant. R = upregulated, L= downregulared
green = stat insignificant,  R = upregulated, L= downregulared

## Use this link and modify your Volcano Plot, submit below
